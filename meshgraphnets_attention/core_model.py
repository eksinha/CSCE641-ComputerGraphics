...
class GraphNetBlock(snt.AbstractModule):
  ...
  def __init__(self, model_fn, size_edge, name='GraphNetBlock'):
    super(GraphNetBlock, self).__init__(name=name)
    self._model_fn = model_fn
    self._num_edges = size_edge
    self._model_fn_alpha = self._attention_MLP([self._num_edges]*3, self._num_edges)

  def _attention_MLP(self, layers, output_size):
    widths = layers + [output_size]
  ...
  def _update_node_features(self, node_features, edge_sets):
    ...
    for edge_set in edge_sets:
      sender_features = tf.gather(node_features, edge_set.senders)
      receiver_features = tf.gather(node_features, edge_set.receivers)
      a = self._model_fn_alpha(receiver_features) #latent_size
      b = self._model_fn_alpha(sender_features) #latent_size
      alpha = tf.math.exp(tf.reduce_sum(tf.math.l2_normalize(a*b), axis=-1, keepdims=True)) #latent_size
      weighted_sum = tf.unsorted_segment_sum(edge_set.features*alpha, edge_set.receivers, num_nodes)
      sum_weights = tf.unsorted_segment_sum(alpha, edge_set.receivers, num_nodes)
...
class EncodeProcessDecode(snt.AbstractModule):
  ...
  def _build(self, graph):
    ...
    for _ in range(self._message_passing_steps):
      latent_graph = GraphNetBlock(model_fn, self._latent_size)(latent_graph)
